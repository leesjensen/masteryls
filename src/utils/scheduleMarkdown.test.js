...
`;
  ...
  assert.equal(model.weeks[0].dueItems[0].href, '../instruction/startup.md');
  assert.equal(model.weeks[0].topicsCovered[1].text, 'Read syllabus');
    ...
    weeks: [
      {
        ...
        date: 'Tue Jan 5',
        module: 'Intro',
        dueItems: [{ id: 'd1', text: 'Project Setup', href: '../instruction/setup.md' }],
        topicsCovered: [{ id: 't1', text: 'Welcome', href: '../instruction/welcome.md' }],
        slides: [{ id: 's1', text: 'Kickoff', href: 'https://slides.example.com' }],
      },
    ],
...
test('parseCellItems supports checkbox-prefixed links without storing checked state', () => {
  const cell = '☑ [A](../a.md)</br>- [x] [B](../b.md)</br>Plain note';
  const items = parseCellItems(cell);

  assert.equal(items.length, 3);
  assert.equal(items[0].text, 'A');
  assert.equal(items[0].href, '../a.md');
...
`;
  ...
  const serialized = serializeScheduleMarkdown({
    ...
    links: parsed.links,
    weeks: [
      { ...parsed.weeks[0], week: 777 },
      { ...parsed.weeks[1], week: -3 },
    ],
