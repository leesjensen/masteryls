...
export default function Dashboard({ user }) {
  const courseList = Array.isArray(user.courses) ? user.courses : [];
      ...
      {courseList.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {courseList.map((course) => {
            ...
            return (
              <button key={course.id} type="button" onClick={() => setSelectedCourseId(course.id)} className={`flex flex-col items-center p-6 rounded-xl bg-gray-50 shadow-md min-h-[280px] transition-transform duration-200 focus:outline-none ${isSelected ? 'ring-4 ring-blue-400 scale-105' : 'hover:scale-105 hover:shadow-lg'}`} style={{ cursor: 'pointer' }}>
