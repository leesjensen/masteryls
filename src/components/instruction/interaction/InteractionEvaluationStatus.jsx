...
export function InteractionSubmitRow({ id, details, label = 'Submit', loadingLabel = 'Evaluating...', disabled = false, buttonClassName = '', buttonProps = {}, children, className = '' }) {
  ...
  return (
    <div className={`mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 ${className}`.trim()}>
      <button id={`submit-${id}`} type="submit" className={`px-6 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-600 transition-colors duration-200 ${buttonClassName} ${buttonPropsClassName}`.trim()} disabled={isLoading || disabled || buttonPropsDisabled} {...restButtonProps}>
...
function InteractionEvaluationStatus({ details }) {
  ...
  return (
    <div className="flex items-center gap-3 text-blue-900 animate-pulse" role="status" aria-live="polite">
