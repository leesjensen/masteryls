      ...
      {[0, 1, 2, 3, 4].map((index) => {
        ...
        return (
          <span key={index} className="relative inline-block">
            <Star className={`${sizeClass} text-slate-400`} strokeWidth={1.8} fill="white" />
            <Star className={`absolute inset-0 ${sizeClass} text-amber-400`} strokeWidth={1.8} fill="currentColor" stroke="currentColor" style={{ clipPath: `inset(0 ${clippedRight}% 0 0)` }} />
          </span>
        );
      })}
    </div>
