...
function TopicForm({ title, type, onSubmit, onCancel, isLoading }) {
  ...
  const handleKeyDown = (e) => {
    if (isLoading) return; // Block keyboard actions during loading
  ...
  return (
    <div className="mb-0.5 ml-4 p-2 bg-gray-50 border rounded relative">
      ...
      <div className="flex flex-col gap-2">
        <input type="text" placeholder="Topic title" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} onKeyDown={handleKeyDown} className="px-2 py-1 border rounded text-sm disabled:bg-gray-100 disabled:cursor-not-allowed" autoFocus disabled={isLoading} />
        <input type="text" placeholder="Topic description" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} onKeyDown={handleKeyDown} className="px-2 py-1 border rounded text-sm disabled:bg-gray-100 disabled:cursor-not-allowed" disabled={isLoading} />
